...
// // --- Prompt demo (frontend only) ---
// const promptInput = document.getElementById('promptInput');
// const promptBtn = document.getElementById('promptBtn');
// const promptOut = document.getElementById('promptOut');

// // Pages map (single source of truth)
// const ROUTES = [
//   { page: 'index.html',    keys: ['home', 'main', 'start', 'landing'] },
//   { page: 'about.html',    keys: ['about', 'bio', 'me', 'who are you', 'profile'] },
//   { page: 'projects.html', keys: ['project', 'projects', 'portfolio', 'builds', 'work samples'] },
//   { page: 'work.html',     keys: ['work', 'experience', 'featured', 'content', 'community', 'connect'] },
//   { page: 'contact.html',  keys: ['contact', 'email', 'reach', 'message', 'call', 'connect with you'] },
// ];
...
// // Phase 1: exact single-word commands
// function matchPhase1(cmd) {
//   const t = normalize(cmd);
//   // allow "project" or "projects" etc
//   for (const r of ROUTES) {
//     for (const k of r.keys) {
//       if (t === k) return r.page;
//     }
//   }
//   return null;
// }

// // Phase 2: intent + keyword match inside a sentence
// function matchPhase2(sentence) {
//   const t = normalize(sentence);

//   // Optional: require navigation intent words (to reduce false matches)
//   const navIntents = ['go', 'take me', 'open', 'show', 'navigate', 'bring me', 'redirect', 'visit'];
//   const hasIntent = navIntents.some(w => t.includes(w));

//   // If you want strict: only redirect when intent exists
//   // If you want loose: comment next 2 lines
//   if (!hasIntent) return null;

//   // score each route by keyword hits
...
//     }
//     if (score > best.score) best = { page: r.page, score };
//   }
...
//   if (promptOut) {
//     promptOut.style.display = 'block';
//     promptOut.textContent = `Redirecting to ${page}...`;
//   }
//   // redirect
//   window.location.href = page;
// }

// // Your existing demo output (keep if you want)
// function fakeRunPrompt(text){
//   const t = (text || '').trim();
//   if (!t) return 'Type a prompt above. Example: “Take me to projects.”';

//   return [
//     'Demo output (frontend only):',
//     `• Interpreted request: ${t.length > 90 ? t.slice(0, 90) + '…' : t}`,
//     '• Next step: retrieval → rerank → answer with citations',
//     '• Guardrails: confidence threshold + abstain when evidence is weak',
//     '• Delivery: API-ready response format'
//   ].join('\n');
// }

// if (promptBtn && promptInput && promptOut) {
//   promptBtn.addEventListener('click', () => {
//     const raw = promptInput.value;

//     // Phase 1 (exact commands)
//     const p1 = matchPhase1(raw);
//     if (p1) return redirectTo(p1);

//     // Phase 2 (sentence navigation)
//     const p2 = matchPhase2(raw);
//     if (p2) return redirectTo(p2);

//     // Otherwise fallback to demo output
//     const out = fakeRunPrompt(raw);
//     promptOut.style.display = 'block';
...
(function eduAdjacentPanel(){
  const cards = document.querySelectorAll('.edu-card');
  const panel = document.getElementById('eduPanel');
  const stage = document.getElementById('eduStage');
  const list  = document.getElementById('eduList');
  const slot  = document.getElementById('eduSelectedSlot');
  const right = document.querySelector('.eduRight'); // list’s original parent

  if (!cards.length || !panel || !stage || !list || !slot || !right) return;

  const t = document.getElementById('eduPTitle');
  const s = document.getElementById('eduPSub');
  const b = document.getElementById('eduPBody');
  const tags = document.getElementById('eduPTags');
  const close = document.getElementById('eduClose');

  let active = null;

  function render(card){
    t.textContent = card.dataset.title || 'Education';
    s.textContent = card.dataset.subtitle || '';
    b.textContent = card.dataset.body || '';
    tags.innerHTML = '';
  ...
  function dockListLeft(){
    if (list.parentElement !== slot) slot.appendChild(list);
    stage.classList.add('expanded');
  }

  function undockListRight(){
    if (list.parentElement !== right) right.appendChild(list);
  ...
  cards.forEach(card => {
    card.addEventListener('click', () => {
      if (active === card && panel.classList.contains('open')) closePanel();
      else open(card);
    });
  });
...
(() => {
  ...
  const closeBtn = document.getElementById('eduMClose');

  const t = document.getElementById('eduMTitle');
  const s = document.getElementById('eduMSub');
  const tags = document.getElementById('eduMTags');
  const body = document.getElementById('eduMBody');
  ...
  function open(card){
    t.textContent = card.dataset.title || 'Education';
    s.textContent = card.dataset.subtitle || '';
    const raw = card.dataset.body || '';
    body.innerHTML = raw
      .replace(/(Coursework:)/g, '<strong>$1</strong>')
      .replace(/(Research Focus:)/g, '<strong>$1</strong>')
      .replace(/(Highlights:)/g, '<strong>$1</strong>');
